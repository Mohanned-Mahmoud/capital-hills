...
function BrowseShell() {
  ...
  return <main className="grain h-[100dvh] overflow-hidden bg-[#f6f0e4] app-reveal flex flex-col"><header className="relative z-20 flex h-[76px] shrink-0 items-center justify-between border-b border-[#4b1e2d]/10 px-5 md:px-10 lg:px-14 bg-[#f6f0e4]"><div><Logo /></div><nav className="hidden lg:flex items-center gap-8">{navItems.map(({ id, label }) => <button key={id} type="button" data-testid={`button-header-nav-${id}`} onClick={() => setView(id)} className={`text-xs uppercase tracking-widest font-semibold transition-colors ${view === id ? 'text-[#c9a36a]' : 'text-[#4b1e2d]/70 hover:text-[#4b1e2d]'}`}>{label}</button>)}</nav><div className="flex items-center gap-2"><button type="button" data-testid="button-header-search" onClick={() => setView('projects')} aria-label="Search projects" className="grid size-9 place-items-center rounded-full text-[#4b1e2d] hover:bg-[#eadfce]"><Search size={17} /></button></div></header><section className="relative min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto"><img src={capitalHillsMark} alt="" aria-hidden="true" className="watermark watermark-dark pointer-events-none absolute -right-24 top-[18%] z-0 size-[360px] object-contain lg:size-[520px]" /><div key={view} className="relative z-10 screen-content px-5 pb-8 pt-8 md:px-10 md:pt-12 lg:px-14">{view === 'home' && <HomeView onOpenProject={setSelectedProject} savedIds={savedIds} onToggleSaved={toggleSaved} onBrowse={() => setView('projects')} onFeedback={notify} />}{view === 'projects' && <ProjectsView onOpenProject={setSelectedProject} savedIds={savedIds} onToggleSaved={toggleSaved} />}{view === 'saved' && <SavedView savedIds={savedIds} onToggleSaved={toggleSaved} onOpenProject={setSelectedProject} onBrowse={() => setView('projects')} />}{view === 'messages' && <MessagesView />}</div></section><nav className="safe-bottom fixed bottom-0 left-0 right-0 z-30 flex border-t border-[#4b1e2d]/10 bg-[#f6f0e4]/95 px-2 pt-2 backdrop-blur-lg lg:hidden">{navItems.map(({ id, label, icon: Icon }) => <button key={id} type="button" data-testid={`button-mobile-${id}`} onClick={() => setView(id)} className={`relative flex flex-1 flex-col items-center gap-1 py-1.5 text-[9px] transition-colors ${view === id ? 'font-semibold text-[#4b1e2d]' : 'text-[#4b1e2d]/45'}`}><Icon size={18} strokeWidth={view === id ? 2 : 1.5} />{label}{view === id ? <span className="absolute -top-2 size-1 rounded-full bg-[#c9a36a]" /> : null}</button>)}</nav><ContactFab />{feedback ? <Toast message={feedback} onClose={() => setFeedback('')} /> : null}</main>;
}
