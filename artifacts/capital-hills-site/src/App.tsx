...
type View = 'home' | 'projects' | 'saved' | 'messages';
...
function ProjectCard({ project, saved, onToggleSaved, onOpen, compact = false }: { project: Project; saved: boolean; onToggleSaved: () => void; onOpen: () => void; compact?: boolean }) {
  return (
    <article data-testid={`card-project-${project.id}`} onClick={onOpen} className="group overflow-hidden rounded-[18px] border border-[#4b1e2d]/10 bg-[#faf6ef] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_28px_rgba(75,30,45,.11)] cursor-pointer">
      <div className={`relative overflow-hidden ${compact ? 'aspect-[1.48]' : 'aspect-[1.18]'}`}>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#26131b]/45 to-transparent pointer-events-none" />
        <span className="absolute left-3 top-3 rounded-full bg-[#f6f0e4]/90 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[.11em] text-[#4b1e2d] z-10">{project.status}</span>
        <button type="button" data-testid={`button-save-project-${project.id}`} aria-label={saved ? `Remove ${project.name} from saved` : `Save ${project.name}`} onClick={(event) => { event.stopPropagation(); onToggleSaved(); }} className={`absolute right-3 top-3 grid size-8 place-items-center rounded-full transition-colors z-20 ${saved ? 'bg-[#c9a36a] text-[#4b1e2d]' : 'bg-[#f6f0e4]/85 text-[#4b1e2d] hover:bg-[#c9a36a]'}`}>
          <Heart size={15} fill={saved ? 'currentColor' : 'none'} />
        </button>
        <div className="absolute bottom-3 right-3 grid size-8 place-items-center rounded-full bg-[#f6f0e4] text-[#4b1e2d] opacity-0 transition-all group-hover:opacity-100 group-focus-within:opacity-100 z-10">
          <ArrowRight size={15} />
        </div>
      </div>
      <div className="w-full px-4 py-3.5 text-left">
...
function JournalSection({ onBrowse, onFeedback }: { onBrowse: () => void; onFeedback: (message: string) => void }) {
  return (
    <section className="animate-rise delay-5 lg:col-span-2 mt-2">
      ...
      <div className="grid gap-3">
        {articles.map((article) => (
          <div key={article.id} onClick={() => onFeedback('Full article view coming soon.')} className="flex items-center justify-between rounded-[16px] bg-[#eadfce]/40 p-4 hover:bg-[#eadfce]/70 transition-colors cursor-pointer">
            <div>
...
function HomeView({ onOpenProject, savedIds, onToggleSaved, onBrowse, onFeedback }: { onOpenProject: (project: Project) => void; savedIds: string[]; onToggleSaved: (id: string) => void; onBrowse: () => void; onFeedback: (message: string) => void }) {
  return (
    <div className="space-y-6 pb-4 md:space-y-8 lg:grid lg:grid-cols-2 lg:gap-x-10 lg:gap-y-8 lg:space-y-0 lg:pb-8">
      ...
      <TestimonialsSection />
      <JournalSection onBrowse={onBrowse} onFeedback={onFeedback} />
    </div>
  );
}
