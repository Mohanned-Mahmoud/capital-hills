...
function MessagesView({ onFeedback }: { onFeedback: (message: string) => void }) {
  return <div className="space-y-6 pb-8"><header className="animate-rise flex items-center justify-between"><div><p className="mono text-[9px] uppercase tracking-[.2em] text-[#4b1e2d]/50">Stay in touch</p><h1 className="serif mt-1 text-[38px] leading-none text-[#4b1e2d]">Messages</h1></div><button type="button" data-testid="button-new-message" onClick={() => onFeedback('A new conversation is ready to begin.')} aria-label="Start a new message" className="grid size-10 place-items-center rounded-full bg-[#4b1e2d] text-[#f6f0e4]"><Send size={16} /></button></header><article className="animate-rise delay-1 rounded-[20px] border border-[#4b1e2d]/12 bg-[#faf6ef] p-5"><div className="flex items-start gap-3"><div className="grid size-11 place-items-center rounded-full bg-[#4b1e2d] text-[#c9a36a]"><span className="serif text-xl">C</span></div><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><div><p className="text-xs font-semibold text-[#4b1e2d]">Capital Hills team</p><p className="mt-0.5 text-[10px] text-[#4b1e2d]/45">Yesterday, 10:42 AM</p></div><span className="size-2 rounded-full bg-[#c9a36a]" /></div><p className="mt-4 text-sm leading-6 text-[#4b1e2d]/70">Welcome to a better way of finding home. We’re here when you’re ready to explore La Colina East.</p><button type="button" data-testid="button-reply-message" onClick={() => onFeedback('Reply window opened.')} className="mt-5 flex items-center gap-2 text-xs font-semibold text-[#4b1e2d]">Reply <ArrowRight size={14} /></button></div></div></article><div className="rounded-[20px] border border-dashed border-[#4b1e2d]/18 px-5 py-12 text-center"><MessageCircle size={21} className="mx-auto text-[#c9a36a]" /><p className="serif mt-3 text-2xl text-[#4b1e2d]">A thoughtful conversation is never far away.</p><p className="mx-auto mt-2 max-w-[260px] text-xs leading-5 text-[#4b1e2d]/50">Ask us about availability, payment plans, or a private viewing.</p></div></div>;
}

function ProfileView({ onFeedback }: { onFeedback: (message: string) => void }) {
  return <div className="space-y-6 pb-8"><header className="animate-rise"><p className="mono text-[9px] uppercase tracking-[.2em] text-[#4b1e2d]/50">Your account</p><h1 className="serif mt-1 text-[38px] leading-none text-[#4b1e2d]">Profile</h1></header><div className="animate-rise delay-1 flex items-center gap-4 rounded-[20px] border border-[#4b1e2d]/10 bg-[#faf6ef] p-5"><div className="grid size-14 place-items-center rounded-full bg-[#eadfce] text-[#4b1e2d]"><span className="serif text-2xl">M</span></div><div><p className="text-sm font-semibold text-[#4b1e2d]">Mohanned Ahmed</p><p className="mt-1 text-xs text-[#4b1e2d]/50">mohanned@capitalhills.com</p></div><button type="button" data-testid="button-edit-profile" onClick={() => onFeedback('Profile editing will be available here.')} className="ml-auto text-[10px] font-semibold text-[#4b1e2d] underline underline-offset-4">Edit</button></div><div className="divide-y divide-[#4b1e2d]/10 overflow-hidden rounded-[20px] border border-[#4b1e2d]/10 bg-[#faf6ef]">{[['Viewing preferences', 'New Cairo · Apartments'], ['Saved searches', '2 active searches'], ['Notifications', 'On for new launches']].map(([label, value]) => <button type="button" data-testid={`button-profile-${label.toLowerCase().replaceAll(' ', '-')}`} onClick={() => onFeedback(`${label} opened.`)} key={label} className="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-[#eadfce]/60"><span><span className="block text-xs font-semibold text-[#4b1e2d]">{label}</span><span className="mt-1 block text-[10px] text-[#4b1e2d]/50">{value}</span></span><ChevronRight size={15} className="text-[#4b1e2d]/40" /></button>)}</div><button type="button" data-testid="button-sign-out" onClick={() => onFeedback('You are still signed in on this device.')} className="w-full rounded-full border border-[#4b1e2d]/15 py-3 text-xs font-semibold text-[#4b1e2d] hover:bg-[#eadfce]">Sign out</button></div>;
}
